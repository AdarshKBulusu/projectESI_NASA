...
M_indexes = [] #list to hold the values to where Type A planets area
for x in df['st_spstr']:
    if(re.search("M",str(x)[0])):
        pos=df[df['st_spstr'] == str(x)].index[0]
        M_indexes.append(pos)
        
f = open('/Users/adarshbulusu/Desktop/FRP /TypeM.csv','w')
with f:
    writer = csv.writer(f)
    writer.writerow(df.columns)
    for val in M_indexes:
